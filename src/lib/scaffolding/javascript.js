...
async function setupPackageJson(argv, facts) {
      ...
      .then((pkg) => {
        ...
        try {
          shift.api.setOrReplaceScript(pkg, {
            name: 'lint',
            to:
              'npm run --silent lint:js && npm run --silent lint:changelog && npm run --silent lint:deps'
          });
        } catch (err) {
        ...
        try {
          shift.api.setOrReplaceScript(pkg, {
            name: 'lint:deps',
            to:
              'npm run --silent lint:deps:missing && npm run --silent lint:deps:unused'
          });
        } catch (err) {
        ...
        try {
          shift.api.setOrReplaceScript(pkg, {
            name: 'test',
            to:
              "mocha $(npm run --silent mocha-reporter-options) './src/**/*-spec.js'"
          });
        } catch (err) {
        ...
        try {
          shift.api.setOrReplaceScript(pkg, {
            ...
            to:
              ...
              '[ -n "${CI}" ] && echo \'--reporter xunit --reporter-options output=reports/junit/mocha.xml\''
          });
        } catch (err) {
  ...
  if (argv.coverage) {
    await addToGitIgnore(['.nyc_output', 'coverage']);

    debug('installing code coverage depencies');
    await npmInstallDev(['coveralls', 'nyc']);
